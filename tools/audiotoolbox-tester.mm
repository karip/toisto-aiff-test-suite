...
int main(int argc, const char * argv[]) {
    ...
    AudioFileMarkerList *markers = (AudioFileMarkerList*)(markersBuffer);
    markers->mNumberMarkers = 0;
    err = AudioFileGetProperty(fileID, kAudioFilePropertyMarkerList, &msize, markers);
    if (err != noErr) {
        printerr("Failed to read kAudioFilePropertyMarkerList\n");
        exit(-1);
    }
    ...
    // TODO: read id3 tags directly as a property?
    // getting kAudioFilePropertyID3Tag returns kAudioFileUnsupportedPropertyError ('pty?')
    UInt32 id3size = getPropertySize(fileID, kAudioFilePropertyID3Tag);
    char *rawID3Tag = (char *)malloc(id3size);
    err = AudioFileGetProperty(fileID, kAudioFilePropertyID3Tag, &id3size, rawID3Tag);
    if (err != noErr) {
        printerr("Failed to read id3 tag\n");
        exit(-1);
    }

    UInt32 id3TagSize = 0;
    UInt32 id3TagSizeLength = 0;
    err = AudioFormatGetProperty(kAudioFormatProperty_ID3TagSize, id3size, rawID3Tag,
            &id3TagSizeLength, &id3TagSize);
    if (err != noErr) {
        printerr("Failed to read id3 format\n");
        exit(-1);
    }
#endif

    // get metadata
    // hacky way to extract anno, name, id3 etc. values, because they
    // all are mapped to InfoDictionary
    CFDictionaryRef piDict = nil;
    UInt32 dsize = getPropertySize(fileID, kAudioFilePropertyInfoDictionary);
    err = AudioFileGetProperty(fileID, kAudioFilePropertyInfoDictionary, &dsize, &piDict);
    if (err != noErr) {
        printerr("Failed to read kAudioFilePropertyInfoDictionary\n");
        exit(-1);
    }
    //NSLog(@"got metadata: %@\n", piDict);
    if (CFDictionaryGetCount(piDict) > 1) {
        ...
        tboxKeyToId3Key.insert(std::make_pair("genre", "TCO"));
        tboxKeyToId3Key.insert(std::make_pair("title", "ATT2"));
        tboxKeyToId3Key.insert(std::make_pair("track number", "TRK"));
        tboxKeyToId3Key.insert(std::make_pair("year", "TYE"));
        tboxKeyToId3Key.insert(std::make_pair("comments", "COM"));
        std::map<std::string, std::string>::iterator it;
        bool first = true;
        for (it = keyValues.begin(); it != keyValues.end(); it++) {
            std::string key = it->first;
            std::string value = it->second;
            if (key == "approximate duration in seconds") {
                continue;
            }
            if (!first) {
                printf(",\n");
